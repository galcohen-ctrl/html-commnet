(function(){
  ...
  var PAGE='app-builder-p1';
  var PROJECT='app-builder-p1';
  var DATA_BRANCH='data';
  var ASSET_DIR='comment-assets/app-builder-p1';
  ...
  function chooseAnchor(el){
    ...
    var meaningful=[
      '.modal','.phone','.pscreen','.cp-widget-row','.cp-detail','.cp-radio-group','.cp-color-hex',
  ...
  function selectorFor(el){
    ...
    var attrs=['data-widget','data-detail','data-bind','data-bind-radio','data-value','id'];
    var candidates=[];
  ...
  function createIssue(meta){
    ...
    return fetch('https://api.github.com/repos/'+REPO+'/issues',{
      method:'POST',
      headers:Object.assign(ghHeaders(),{'Content-Type':'application/json'}),
      body:JSON.stringify({title:title,body:issueBody(meta),labels:['web-comment','app-builder-p1']})
  ...
  function openBubble(c,btn){
    ...
    bub.innerHTML='<div class="hc-cb-name">'+escH(c.name||'Reviewer')+'</div>'
      +(c.target&&c.target.selector?'<div class="hc-cb-ctx">'+escH(c.target.selector)+'</div>':'')
